        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE): return
                    ...
                    if event.button == 3: scenario_manager.next_scenario()
                elif event.type == pygame.JOYHATMOTION:
                    # Assuming single D-pad hat (hat index 0)
                    hat_x, hat_y = event.value
                    if hat_y == -1 and adas_state in [ADAS_State.STANDBY]:
                        # D-pad DOWN
                        target_speed_kph, adas_state = current_speed_kph, ADAS_State.ACTIVE
                    elif hat_y == 1 and adas_state == ADAS_State.STANDBY:
                        # D-pad UP
