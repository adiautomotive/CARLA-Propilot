...
import Propilot_simulator_dev_copy as simulation_main  # your simulation script
import random
import time
from datetime import datetime
import requests
import carla
from utils import to_human_readable, to_vehicle_code, get_weather_parameter
...
class ADASSimulatorGUI(tk.Tk):
    def __init__(self):
        ...
        self.frames = {}
        StartMenu.client = carla.Client('localhost', 2000)  # Connect to CARLA server
        StartMenu.client.set_timeout(10.0)
        print("Loading map: %s..." % 'Town04')
        StartMenu.client.load_world('Town04')
        print("Map loaded.")
        for F in (StartMenu, DashboardScreen):
            frame = F(parent=self.container, controller=self, client=StartMenu.client)
            self.frames[F] = frame
    ...
    def run_simulation(self, params):
        ...
        try:
            ...
            self.frames[DashboardScreen].vehicle_state = self.vehicle_state
            simulation_main.main(self, params)
...
class StartMenu(tk.Frame):
    def __init__(self, parent, controller,client):
        ...
        # === Background Image ===
        bg_image = Image.open("/home/jesudara/carla_dev/carla/scripts/ADASProject2/CARLA-Propilot/main/images/MeetingCritical.png")
        ...
        # self.configure(bg="#001B40")  # Set background color

        self.client = client
        self.widgets = []
        # Fetch available weather options and vehicles
    ...
    def init_styles(self):
        ...
        style.configure("TLabel", background="#001B40", foreground="white")
        style.configure("TCheckbutton", background="#001B40", foreground="white")
    ...
    def create_widgets(self):
        ...
        top_y, step_y,step_x = 0.15, 0.07, 0.07
        font_inline = font.Font(size=14)

        # Variables
        self.traffic_vehicles = tk.IntVar(value=10)
        self.pedestrians = tk.IntVar(value=5)
        tk.Label(self, text="University of Michigan ADAS Simulator", font=("Times", 20, "normal" ), bg="#001F44", fg='white').place(relx=0.5, rely=0.05, anchor='center')
        self._fetch_vehicles()
        vehicles_names = to_human_readable(self.vehicles)
        self.selected_player_vehicle = to_human_readable(self.selected_player_vehicle)
        self.lead_vehicle_name = to_human_readable(self.selected_lead_vehicle)
        self.lead_vehicle_name = tk.StringVar(value=self.lead_vehicle_name)
        self.selected_player_vehicle = tk.StringVar(value=self.selected_player_vehicle)
        self.weather = tk.StringVar(value="Clear")
        self.scenario = tk.StringVar(value="Baseline")
        
        inputs = [
            (left_x, "Player Vehicle:", self.selected_player_vehicle, vehicles_names),
            (right_x, "Lead Vehicle:", self.lead_vehicle_name, vehicles_names),
            (left_x, "Initial Weather:", self.weather, ["Clear", "Light Rain", "Heavy Rain", "Foggy"]),
            (right_x, "Initial Scenario:", self.scenario, ["Baseline", "Degraded Lanes", "Cut-in Vehicle", "Stop-and-Go"]),
        ]
        ...
        for i, (x, label, var, options) in enumerate(inputs):
            if x == left_x:
                ...
                ttk.Label(self, text=label, font=font_inline).place(relx=x, rely=top_y + i*step_y - step_x, anchor='center')
            ...
            if x == left_x:
                ...
                cb.place(relx=x, rely=top_y + i*step_y - 0.04, anchor='center')
        ...
        for i, (label, var) in enumerate(entry_fields, start=4):
            if i == 4:
                ...
                ttk.Label(self, text=label, font=font_inline).place(relx=left_x, rely=top_y + i * step_y - step_x, anchor='center')
                entry = ttk.Entry(self, textvariable=var)
                entry.place(relx=left_x, rely=top_y + i * step_y - 0.04, anchor='center')
                self.widgets.append(entry)
    ...
    def start_simulation(self):
        params = {
            "vehicle_model": to_vehicle_code(self.selected_player_vehicle.get()),
            "lead_vehicle_model": to_vehicle_code(self.lead_vehicle_name.get()),
            "weather": get_weather_parameter(self.weather.get(), self.weather_options),
            "host": "localhost",
            "port": 2000
        }
...
class DashboardScreen(tk.Frame):
    def __init__(self, parent, controller, client):
        ...
        from dashboard_state import ADAS_State
        ...
        # ProPILOT State
        self.propilot_state = ADAS_State.OFF  # ranges 0–4
    ...
    def update_dashboard(self):
        if hasattr(self, 'vehicle_state') and self.vehicle_state:
        ...
        mode_labels = [
            ...
            "Hands-Off Mode", "ProPILOT Assist"
        ]
        mode_color = mode_colors[self.propilot_state.value]
        mode_text = mode_labels[self.propilot_state.value]
    ...
    def update_from_vehicle(self):
        import math
        if hasattr(self, 'vehicle_state') and self.vehicle_state:
            vs = self.vehicle_state
            self.speed += (vs.current_speed_kph - self.speed) * 0.3
            self.rpm += (vs.rpm - self.rpm) * 0.3
            gear_map = {-1: "R", 0: "N", 1: "D"}
            # Determine gear based on speed and ProPILOT timer state
            if self.speed > 0.5:
                self.gear = "D"
            elif self.speed < -0.5:
                self.gear = "R"
            elif abs(self.speed) <= 0.5:
                if self.propilot_state in [0, 1]:
                    self.gear = "P"
                else:
                    # Keep gear unchanged (e.g., still in D if ProPILOT is active)
                    pass
            # === ProPILOT State Machine ===
            self.propilot_timer += 1
            if self.propilot_timer > 20:  # every ~5 seconds
                self.propilot_state = vs.adas_state
                self.propilot_timer = 0
